...
public final class Configuration {
	 ...
	 * number of Passengers to create during the creation/initial population phase
	 * of the TrainSimulation.
	 */
	...
	/**
	 * Utility construct for a pair of corresponding minimum and maximum limits for
	 * a parameter. This class is immutable; no getters are provided.
	 */
	public final class PairedLimit {
		...
		public String toString() {
			return String.format((minimum == maximum ? "%,d" : "%,d..%,d"), minimum, maximum);
		} // end toString()
	...
	public final class RouteSpec {
		 ...
		 */
		public RouteSpec(RouteStyle styleSpec, int lengthSpec) {
			style = styleSpec;
			length = lengthSpec;
		} // end constructor
		...
		@Override
		public String toString() {
			return String.format("%s with length %,d", style.toString(), length);
		} // end toString()
	...
	public final class TrainSpec {
		 ...
		 */
		public TrainSpec(int locationSpec, Direction directionSpec, int capacitySpec) {
		...
		@Override
		public String toString() {
			return String.format("%s at location %,d with capacity %,d", direction.toString(), location, capacity);
		} // end toString()
	} // end class TrainSpec
	 ...
	 * opened, read, then closed. Parsing of the individual specifications is
	 * delayed until the corresponding getXxx() method is invoked.
	 * 
	 * @throws FileNotFoundException if TrainSimulation.config is missing or can't
	 ...
	 *                       folder; if a path is specified, it must be relative to
	 *                       the default location (e.g.
	 *                       "./data/TrainSimulation.config")
	 * 
	 *                       <p>
	 *                       Note: Absolute paths will work but are not acceptable
	 *                       for this lab.
	 * @throws FileNotFoundException if the configuration file is missing or can't
	 *                               be opened for reading
	 */
	public Configuration(String configFilename) throws FileNotFoundException {
		...
		while (input.hasNextLine()) {
			...
			parameterValue = parameterParts.length == 1 ? "" : parameterParts[1].trim();
	...
	public PairedLimit[] getPassengers() {
		...
		String[] specificationLimits = passengerSpecification[PASSENGERS_INITIAL].trim().split(":");
		...
		specificationLimits = passengerSpecification[PASSENGERS_PER_TICK].trim().split(":");
	...
	/**
	 * Parses the value to use to seed the random number generator. The default is
	 * 0, which should be treated as a sentinel value to either (1) not seed the
	 * random number generator or (2) seed it with the current time.
	 * 
